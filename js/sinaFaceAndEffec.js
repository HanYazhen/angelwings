...
window.onload = function () {
    var content_text_list = document.getElementsByClassName("content_text");
    for (var i = 0; i < content_text_list.length; i++) {
        content_text_list[i].innerHTML = AnalyticEmotion(content_text_list[i].innerHTML);
    }
    var content_img_list = document.getElementsByClassName("content_img");
    for (var i = 0; i < content_img_list.length; i++) {
        // alert(content_img_list[i].naturalWidth);
    }
};
...
(function ($) {
    $.fn.SinaEmotion = function (target) {
        ...
        $(this).click(function (event) {
            ...
            var click_id = $(".face-icon").index(this);
            target = $(".text").eq(click_id);
            ...
            $('#emotions').html('<div style="float:right"><a href="javascript:void(0);" id="prev">&laquo;</a><a href="javascript:void(0);" id="next">&raquo;</a></div><div class="categorys"></div><div class="contain"></div>');
        ...
        function showEmotions() {
            ...
            $('#emotions .contain').html('');
            cat_current = category;
            for (var i = 0; i < emotions[category].length; ++i) {
                $('#emotions .contain').append($('<a href="javascript:void(0);" title="' + emotions[category][i].name + '"><img src="' + emotions[category][i].icon + '" alt="' + emotions[category][i].name + '" width="22" height="22" /></a>'));
            }
            $('#emotions .contain a').click(function () {
